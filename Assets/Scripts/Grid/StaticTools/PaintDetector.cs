...
public static class PaintDetector
{
        ...
        out SelectedCellData selectedCellData)
    {
        ...
        if (!GridProjector.TryWorldPositionToCellIndex(hitPointWorld, gridData, out Vector2Int selectedCellIndex))
            return false;

        selectedCellData = new SelectedCellData(selectedCellIndex, isPainting);
